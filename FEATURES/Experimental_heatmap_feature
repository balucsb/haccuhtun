...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: Arial, sans-serif; }
        ...
        .heat-legend { display:flex; align-items:center; gap:12px; font-size:13px; color:#ddd; margin-bottom:8px; }
        .heat-legend input[type="checkbox"] { transform:scale(1.05); margin-right:6px; }
        .heat-legend .heat-swatch { width:36px; height:12px; border-radius:6px; background: linear-gradient(90deg,#2ecc71, #f1c40f, #e74c3c); border:1px solid rgba(255,255,255,0.06); }
        .time-slider-wrap { display:flex; align-items:center; gap:8px; color:#ddd; }
        .time-slider { width:220px; }
        .time-display { min-width:64px; text-align:center; background:#222; padding:6px 8px; border-radius:6px; border:1px solid #333; }
    </style>
</head>
<body>
...
<?php elseif ($step === 3): ?>
    ...
    <div class="map-legend">
        <div class="heat-legend">
            ...
            <div class="heat-swatch" title="heat intensity"></div>
            <div class="time-slider-wrap">
                <input id="time-slider" class="time-slider" type="range" min="0" max="23" step="1" value="<?= htmlspecialchars($simulatedHour === '' ? date('H') : $simulatedHour) ?>">
...
<?php elseif ($step === 4): ?>
    ...
    <div class="details-panel">
        <div style="margin-bottom:10px; color:#ccc; font-size:13px; display:flex; gap:12px; align-items:center;">
            <label style="display:inline-flex; align-items:center; gap:8px;">
                <input type="checkbox" id="route-heat-toggle" checked>
                Show route heat overlay
            </label>
...
// Simulated hour (from PHP fallback to server hour) and mutable current value
const SIM_HOUR = <?= json_encode($simulatedHour === '' ? (int)date('H') : (int)$simulatedHour) ?>;
let currentSimHour = Number(SIM_HOUR);
let overviewHeat = null;
let routeHeat = null;
...
function isRushFor(h) { return (h >= 7 && h <= 9) || (h >= 17 && h <= 20); }

// Simple route-color -> gradient presets (stops: 0..1)
const GRADIENTS = {
    '#5cb85c': { 0.2: '#c8f7d6', 0.6: '#f0ad4e', 1.0: '#d9534f' },
    '#0275d8': { 0.2: '#d6eefb', 0.6: '#5bc0de', 1.0: '#0275d8' },
    '#f0ad4e': { 0.2: '#fff0d6', 0.6: '#ffb84d', 1.0: '#d9534f' },
    '#d9534f': { 0.2: '#ffdede', 0.6: '#ff8a80', 1.0: '#b03a2e' }
};
function getGradientForColor(hex) {
    const key = (hex || '').toLowerCase();
    return GRADIENTS[key] || { 0.2: '#2ecc71', 0.6: '#f1c40f', 1.0: '#e74c3c' };
}

function updateTimeDisplayUI() {
    const disp = document.getElementById('time-display');
    if (disp) disp.textContent = formatHour(currentSimHour);
}

function buildOverviewHeat(map, hour) {
    try {
        if (overviewHeat && map.hasLayer(overviewHeat)) map.removeLayer(overviewHeat);
        const heatData = [];
        ALL_ROUTES.forEach(route => {
            const intensityBase = isRushFor(hour) ? 1.0 : 0.5;
            route.waypoints.forEach(pt => heatData.push([pt[0], pt[1], intensityBase]));
        });
        overviewHeat = L.heatLayer(heatData, {
            radius: 30, blur: 24, maxZoom: 17,
            gradient: getGradientForColor('#5cb85c')
        });
        const heatToggle = document.getElementById('heatmap-toggle');
        if (!heatToggle || heatToggle.checked) overviewHeat.addTo(map);
        if (heatToggle && !heatToggle.dataset.heatInit) {
            heatToggle.dataset.heatInit = '1';
            heatToggle.addEventListener('change', (ev) => {
                if (ev.target.checked) overviewHeat.addTo(map);
                else if (map.hasLayer(overviewHeat)) map.removeLayer(overviewHeat);
            });
        }
    } catch (e) {
        console.warn('buildOverviewHeat failed', e);
    }
}

function buildRouteHeat(map, waypoints, hour, routeColor) {
    try {
        if (routeHeat && map.hasLayer(routeHeat)) map.removeLayer(routeHeat);
        const intensity = isRushFor(hour) ? 1.2 : 0.9;
        const routeHeatData = waypoints.map(pt => [pt[0], pt[1], intensity]);
        routeHeat = L.heatLayer(routeHeatData, { radius: 22, blur: 20, maxZoom: 17, gradient: getGradientForColor(routeColor) });
        const routeToggle = document.getElementById('route-heat-toggle');
        if (!routeToggle || routeToggle.checked) routeHeat.addTo(map);
        if (routeToggle && !routeToggle.dataset.heatInit) {
            routeToggle.dataset.heatInit = '1';
            routeToggle.addEventListener('change', (ev) => {
                if (ev.target.checked) routeHeat.addTo(map);
                else if (map.hasLayer(routeHeat)) map.removeLayer(routeHeat);
            });
        }
    } catch (e) {
        console.warn('buildRouteHeat failed', e);
    }
}

// Wire up slider once (if present) — input may appear in step 3 or 4 depending on page state
function bindTimeSlider(handler) {
    const slider = document.getElementById('time-slider');
    if (!slider) return;
    if (!slider.dataset.init) {
        slider.dataset.init = '1';
        slider.addEventListener('input', (ev) => {
            currentSimHour = parseInt(ev.target.value, 10);
            updateTimeDisplayUI();
            handler(currentSimHour);
        });
    }
    updateTimeDisplayUI();
}
...
if (CURRENT_STEP === 3 && document.getElementById('overview-map')) {
    ...
    // Build overview heat layer for the current simulated hour
    buildOverviewHeat(map, currentSimHour);
    // bind slider to rebuild overview heat when changed
    bindTimeSlider((hour) => buildOverviewHeat(map, hour));
}
...
if (CURRENT_STEP === 4 && SELECTED_ROUTE && document.getElementById('route-map')) {
    ...
    // Route-specific heat overlay using current simulated hour and route gradient
    buildRouteHeat(map, waypoints, currentSimHour, color);
    bindTimeSlider((hour) => buildRouteHeat(map, waypoints, hour, color));
}
</script>
